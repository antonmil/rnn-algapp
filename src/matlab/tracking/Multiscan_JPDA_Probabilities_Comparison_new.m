function [Final_probabilty,diff,NVHypo,TNH, TimeComp]=Multiscan_JPDA_Probabilities_Comparison_new(M,Obj_info,mbest)
...
for i=unique(C2)
    ...
    [Final_probabilty2, tends,tends_naive]=MBest_JPDA_Probabilty_CalculatorMulti_new(Obj_info(ix),min(max(mbest),NVHypo(i)));
        TimeComp{1,i}=alltime';
        TimeComp{2,i}=tends;
        TimeComp{3,i}=tends_naive;
