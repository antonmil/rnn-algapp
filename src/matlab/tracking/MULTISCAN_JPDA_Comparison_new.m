function [XeT,PeT,Xe,Pe,Ff,Term_Con,mui,diff, NVHypo,TNH,TimeComp]=MULTISCAN_JPDA_Comparison_new(XYZ,F,Q,H,R,X0,P0,Tracking_Scheme,JPDA_P,mbest,...
...
NVHypo=cell(Kt,Frame);
TNH=cell(Kt,Frame);
diff=cell(Frame,Kt);
TimeComp=cell(Kt,Frame);
...
for f=2:Frame
    ...
    for r=1:Kt
        ...
        if strcmp(Tracking_Scheme,'JPDA')
            ...
            [Final_probabilty{1,r}(1,exist_ind),diff{f,r},NVHypo{r,f},TNH{r,f},TimeComp{r,f}] =Multiscan_JPDA_Probabilities_Comparison_new(Mes_Tar(:,:,r),Rt_In_Pr(exist_ind,r),mbest);
