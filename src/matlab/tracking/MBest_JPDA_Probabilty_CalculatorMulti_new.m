function [F_Pr1, times,times_n]=MBest_JPDA_Probabilty_CalculatorMulti_new(M,N)

N_T=size(M,1); % Number of targets in the cluster
F_Pr1 = cell(N,1);
F_Pr=cell(N,N_T);% Final Probabilities
...
tic
[candidates, values, times] = BinIntMBest(f,A,b,Aeq,beq,options,N);
[~, ~, times_n] = CbasedBinIntMBest(f,A,b,Aeq,beq,options,N);
fprintf('NFramework');
for hc=1:length(values)
    ...
    for m=1:N
        if m>=hc
            for nn=1:N_T
                F_Pr{m,nn}(logical(x2(Edge_vec0{nn})),1)=F_Pr{m,nn}(logical(x2(Edge_vec0{nn})),1)+f_prr;
            end
        end
    end
end
...
for m=1:N
    cpr=cellfun(@(x) x/sum(x), {F_Pr{m,:}}, 'UniformOutput', false);
    F_Pr1{m} = cpr;
end
