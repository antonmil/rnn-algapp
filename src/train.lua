...
opt.nClasses = opt.max_m+opt.max_n
...
modelParams = {'model', 'rnn_size', 'num_layers','max_n','max_m'}
...
--       print(mini)
  ...
  print(probLine)
      ...
      print(predDA)
      print(hun)
      eval_val_mm = getDAErrorHUN(predDA:reshape(opt.max_n,1,opt.nClasses), hun:reshape(opt.max_n,1))
--       print(eval_val_mm)
--       abort()
    end    
...
--       rangeStr = string.format("set yrange [%f:%f]", minY, maxY)
      ...
      local meanmets = torch.ones(1,14) * (-100)
      local meanmetsNOPOS = torch.ones(1,14) * (-100)
      if opt.eval_mot15 ~= 0 then 
  local timer = torch.Timer()
  local trackerScript = 'rnnTrackerBFSEP'
--  if stateVel then trackerScript = 'rnnTrackerV' end
  if opt.eval_mot15 < 0 then 
    meanmets = runMOT15(trackerScript, modelName, modelSign.. '_predupd', valSeqTable)
  else
    meanmets = runMOT15(trackerScript, modelName, modelSign..'_predupd')
  end
  ...
  rangeStr = string.format("set xrange [%d:%d];set yrange [%f:%f]",   opt.eval_val_every-1, i+1, -1, 30)
  ...
  if maxMOTAIt == nil or maxMOTAIt == plot_mota:nElement() then
    ...
    -- all-time best
    if writeBestMOTA(maxMOTA, modelName, modelSign,'bestmotaHUN.txt') then -- update all time best
